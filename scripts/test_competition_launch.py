#!/usr/bin/env python3
"""
Launch: Nav2 (SLAM) + 简化版正赛测试 (相对坐标往返)

启动顺序:
  1. Nav2 导航 (slam=True, 在线建图)
  2. 延迟 10s 后启动 test_competition_simple.py
     脚本内部还会等待 /map + TF + action server 就绪

Usage:
    ros2 launch scripts/test_competition_launch.py
"""
...
def generate_launch_description():
    ...
    test_script = os.path.join(ws_dir, 'scripts', 'test_competition_simple.py')
    ...
    log_info = LogInfo(msg=['[TEST-COMP] Logs → ', log_dir])
    ...
    test_node = TimerAction(
        period=10.0,
        actions=[
            LogInfo(msg='[TEST-COMP] 10s elapsed, 启动简化测试 ...'),
            ExecuteProcess(
                cmd=[
                    'python3', test_script,
