...
from custom_msgs.msg import ReadDJIMotor, WriteDJIMotor, ReadDJIRC, ReadLkMotor, WriteLkMotorTorqueControl
...
class GimbalController(Node):
    def __init__(self):
        ...
        # 读取Topic参数
        topic_rc = self.get_parameter('topic_rc_read').value
        topic_imu = self.get_parameter('topic_imu_read').value
        topic_pitch_w = self.get_parameter('topic_pitch_write').value
        topic_pitch_r = self.get_parameter('topic_pitch_read').value
        topic_yaw_w = self.get_parameter('topic_yaw_write').value
        topic_yaw_r = self.get_parameter('topic_yaw_read').value

        self.sub_rc = self.create_subscription(ReadDJIRC, topic_rc, self.cb_rc, qos_best_effort)
        self.sub_imu = self.create_subscription(Imu, topic_imu, self.cb_imu, qos_best_effort)
        self.sub_pitch = self.create_subscription(ReadDJIMotor, topic_pitch_r, self.cb_pitch_fb, qos_best_effort)
        self.sub_yaw = self.create_subscription(ReadLkMotor, topic_yaw_r, self.cb_yaw_fb, qos_best_effort)

        self.pub_pitch = self.create_publisher(WriteDJIMotor, topic_pitch_w, qos_best_effort)
        self.pub_yaw = self.create_publisher(WriteLkMotorTorqueControl, topic_yaw_w, qos_best_effort)
    ...
    def _declare_params(self):
        ...
        self.declare_parameter('topic_pitch_write', '/ecat/sn4587585/app4/write')
    ...
    def control_loop(self):
        ...
        # 获取最新的限位参数 (支持实时更新)
        pitch_min = self.get_parameter('pitch_min_deg').value
        ...
        # 归一化 Target 到 -PI ~ PI
        self.target_yaw_rad = math.atan2(math.sin(self.target_yaw_rad), math.cos(self.target_yaw_rad))
